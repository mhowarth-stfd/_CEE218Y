...
epa_exemption <- smc_exemption %>% 
  ...
  # filter to only single family units by only including exemption of $0, $5600, $7000
  filter(Exemption %in% c(0,5600,7000))
...
leaflet() %>% 
  ...
  addPolygons(
    ...
    color = "black",
    weight = 0.5,
    layer = "Renter Occupied Single Family"
  ) %>% 
  addPolygons(
    data = epa_exemption %>% 
      filter(`Fiscal Year` == "2018/2019") %>% # can change year plotted here
    ...
    weight = 0.5,
    fillOpacity = 1,
    layer = "Owner Occupied Single Family"
  )
